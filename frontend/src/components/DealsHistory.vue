<template>
      ...
      <div v-else class="space-y-4">
        ...
        >
          ...
          <div class="flex items-center gap-3 text-xs pt-3 border-t border-white/20">
            ...
            <span v-if="deal.status === 'active'" class="text-green-600 font-bold">
              ✅ Активна
            </span>
  ...
  const filteredDeals = computed(() => {
    ...
    if (activeTab.value === 'active') return deals.value.filter(d => d.status === 'active' || d.status === 'proposed')
  ...
  const getPriceColor = (status) => {
    ...
    if (status === 'active') return 'text-[#7000ff]'
