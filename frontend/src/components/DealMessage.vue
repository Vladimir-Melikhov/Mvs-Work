<template>
  ...
  const isConfirmedByMe = computed(() => {
    ...
    // Нужно знать кто я - клиент или воркер
    // Для этого нужно получать deal целиком, но пока упростим
    return props.dealData.client_confirmed || props.dealData.worker_confirmed
  })
